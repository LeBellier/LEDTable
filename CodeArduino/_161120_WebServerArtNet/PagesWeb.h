...
void checkRequest(WiFiClient k, String request) {
	...
	if (request.indexOf("c") != -1) {
		...
		int ledNb = chiffres.indexOf(request.charAt(1)) * 16
				+ chiffres.indexOf(request.charAt(2));
		int r = chiffres.indexOf(request.charAt(3)) * 16
				+ chiffres.indexOf(request.charAt(4));
		int g = chiffres.indexOf(request.charAt(5)) * 16
				+ chiffres.indexOf(request.charAt(6));
		int b = chiffres.indexOf(request.charAt(7)) * 16
				+ chiffres.indexOf(request.charAt(8));
		if (ledNb == 156) {
